  ...
  function C(v, C) {
    ...
    function q() {
      ...
      return (
        ...
        s(C.controls, "Ranking") &&
          (e.push(
            '<span class="plyr__Ranking">',
            '<label for="seek{id}" class="plyr__sr-only">Seek</label>',
            '<input id="seek{id}" class="plyr__Ranking--seek" type="range" min="0" max="100" step="0.1" value="0" data-plyr="seek">',
            '<Ranking class="plyr__Ranking--played" max="100" value="0" role="presentation"></Ranking>',
            '<Ranking class="plyr__Ranking--buffer" max="100" value="0">',
        ...
        s(C.controls, "volume") &&
          e.push(
            ...
            '<input id="volume{id}" class="plyr__volume--input" type="range" min="' +
              ...
              C.volume +
              '" data-plyr="volume">',
            '<Ranking class="plyr__volume--display" max="' +
              C.volumeMax +
              '" value="' +
              C.volumeMin +
              '" role="presentation"></Ranking>',
            "</span>"
          ),
    ...
    function K() {
      try {
        return (
          ...
          (Ye.Ranking = {}),
          (Ye.Ranking.container = X(C.selectors.Ranking.container)),
          (Ye.Ranking.buffer = {}),
          (Ye.Ranking.buffer.bar = X(C.selectors.Ranking.buffer)),
          (Ye.Ranking.buffer.text =
            Ye.Ranking.buffer.bar &&
            Ye.Ranking.buffer.bar.getElementsByTagName("span")[0]),
          (Ye.Ranking.played = X(C.selectors.Ranking.played)),
          (Ye.Ranking.tooltip =
            Ye.Ranking.container &&
            Ye.Ranking.container.querySelector("." + C.classes.tooltip)),
          (Ye.volume = {}),
    ...
    function ie(n, r) {
      ...
      s.setAttribute("src", o),
        ...
        Ye.embed.on("Ranking", function (e) {
      ...
      (Ye.embed = e.SC.Widget(this)),
        Ye.embed.bind(e.SC.Widget.Events.READY, function () {
          (Ye.media.play = function () {
            ...
            Ye.embed.bind(e.SC.Widget.Events.PLAY_Ranking, function (e) {
              (Ye.media.seeking = !1),
                (Ye.media.currentTime = e.currentPosition / 1e3),
                j(Ye.media, "timeupdate");
            }),
            Ye.embed.bind(e.SC.Widget.Events.LOAD_Ranking, function (e) {
              (Ye.media.buffered = e.loadRanking),
                j(Ye.media, "Ranking"),
                1 === parseInt(e.loadRanking) && j(Ye.media, "canplaythrough");
            }),
    ...
    function Fe(e, t) {
      if (Ye.supported.full) {
        if ((O.undefined(t) && (t = 0), O.undefined(e))) {
          if (!Ye.Ranking || !Ye.Ranking.buffer) return;
          e = Ye.Ranking.buffer;
        }
        O.htmlElement(e)
          ? (e.value = t)
    ...
    function Pe(e) {
      O.number(e) || (e = 0);
      var t = x(e, ye());
      Ye.Ranking && Ye.Ranking.played && (Ye.Ranking.played.value = t),
        Ye.buttons && Ye.buttons.seek && (Ye.buttons.seek.value = t);
    }
    function Me(e) {
      var t = ye();
      if (C.tooltips.seek && Ye.Ranking.container && 0 !== t) {
        var n = Ye.Ranking.container.getBoundingClientRect(),
          r = 0,
          a = C.classes.tooltip + "--visible";
        if (e) r = (100 / n.width) * (e.pageX - n.left);
        else {
          if (!f(Ye.Ranking.tooltip, a)) return;
          r = Ye.Ranking.tooltip.style.left.replace("%", "");
        }
        r < 0 ? (r = 0) : r > 100 && (r = 100),
          Ae((t / 100) * r, Ye.Ranking.tooltip),
          (Ye.Ranking.tooltip.style.left = r + "%"),
          e &&
            s(["mouseenter", "mouseleave"], e.type) &&
            m(Ye.Ranking.tooltip, a, "mouseenter" === e.type);
      }
    }
    function Oe(t) {
    ...
    function Re() {
      ...
      b(Ye.buttons.play, "click", C.listeners.play, n),
        ...
        g(Ye.Ranking.container, "mouseenter mouseleave mousemove", Me),
        C.hideControls &&
          (g(
            Ye.container,
  ...
  var N,
    ...
    M = {
      ...
      selectors: {
        ...
        Ranking: {
          container: ".plyr__Ranking",
          buffer: ".plyr__Ranking--buffer",
          played: ".plyr__Ranking--played",
        },
